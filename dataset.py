...
class FaceDataset(Dataset):
    def __init__(self, data_dir, data_type, img_size=224, augment=False, age_stddev=1.0):
        ...
        csv_path = Path(data_dir).joinpath(f"encoded_merged_df_{data_type}.csv")
        ...
        for _, row in df.iterrows():
            ...
            self.y.append([row["apparent_age_avg"], row["gender"], row["race"], row["makeup"], row["time"], row["happiness"]])
    ...
    def __getitem__(self, idx):
        ...
        age, gender, race, makeup, time, happiness = self.y[idx]
        ...
        return torch.from_numpy(np.transpose(img, (2, 0, 1))), np.clip(round(age), 0, 100), gender, race, makeup, time, happiness
